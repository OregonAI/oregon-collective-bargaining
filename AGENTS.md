# AGENTS.md — Oregon Collective Bargaining — State and County Labor Agreements
...
## Purpose
Non-authoritative, AI-friendly mirror of the collective bargaining agreements of Oregon state government (DAS Labor Relations) and Oregon county governments, plus the letters of agreement that amend them.
Never a source of truth — every answer must cite and link the
authoritative source.

## The lines this corpus holds

These are decisions recorded in the seed
(`corpus-seeds/oregon-collective-bargaining.md`) and the measured survey beside it;
they are not up for silent revision at ingest time.

1. **No `implements` edges, ever.** A CBA is a negotiated contract, not an
   instrument implementing state law. Statutes an agreement cites (ORS 243 in
   recognition clauses, agency statutes in scope articles) are
   `references_external`, surfaced as `up_cites`. An `implements` claim here is a
   category error the config deliberately does not make.
2. **Summary-first until the class's copyright determination.** `schema.doc_types`
   sets `verbatim: false` for both types on purpose: a CBA is jointly authored with
   private parties, and "public record" ≠ "freely reproducible". Record each
   document's `reproduction_basis`. Flipping a class to verbatim is an operator
   decision made in a PR with the basis stated — never an ingest-time convenience.
3. **Expired ≠ absent; current ≠ assumed.** Every agreement carries `term`,
   `effective_date`, `expiry_date`; a successor records `supersedes`; a superseded
   agreement STAYS, marked `status: superseded`. Never present an expired article
   as current terms; never treat a missing successor as proof none was ratified
   (the survey found one county whose only public copy is five years expired).
4. **Absence discipline runs through `_meta/employers.yml`.** All 36 counties are
   in the registry with a survey status — verified / could-not-verify /
   not-located / not-investigated, kept apart and never summed. "County X's
   agreement is not in this corpus" and "County X does not publish its agreements"
   are different sentences; check the registry before writing either.
5. **The employer's posted copy is the source of record.** Union-posted copies
   (SEIU blacklines, AFSCME's Salesforce index, ONA's CDN) may be more current
   mid-negotiation, but they are corroboration at most. An ERB case exhibit is a
   recorded exception when it is the only public copy — labeled as what it is.
6. **Access, per the house line:** honest User-Agent always; robots directives
   recorded per host in `_meta/sources/<employer>.yml` `crawl:` blocks; WAF
   challenges never evaded. The survey measured one host refusing and then serving
   the same honest UA hours apart — a 403 is `could-not-verify` at that fetch,
   recorded per fetch, never a standing verdict and never an absence.
7. **Content hashing everywhere.** Two verified county publishers (Lane, Marion)
   overwrite undated filenames in place. A pipeline that trusts filenames will
   serve stale terms silently; `corpus-detect-changes` + recorded `sha256` is the
   defence.
...
GitHub Issues on `OregonAI/oregon-collective-bargaining`, via the `gh` CLI. See
